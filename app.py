...
#Produto (id, nome, price, description)
...
#Carrinho de compras (id, user_id, product_id)
...
# Definir uma rota raiz (pagina inicial) e função ao requisitar
@app.route('/')
def hello_world():
    return 'Hello World'
...
def checkout():
    ...
    for cart_item in cart_items:
        db.session.delete(cart_item)
    db.session.commit()
    return jsonify({"message": "Checkout succesful. Cart has been cleared."}), 200
